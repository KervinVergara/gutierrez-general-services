...
export default function AdminQuotes({ goTo }) {
      ...
      <div className="grid gap-4">
        {shown.map((q) => (
          <article key={q.id} className="rounded-2xl bg-white border border-ink/10 p-5">
            <div className="grid md:grid-cols-[1fr_auto] gap-4">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button onClick={() => setExpanded(expanded === q.id ? null : q.id)} className="inline-flex items-center gap-1.5 h-10 px-3 rounded-lg border border-ink/20 text-sm font-bold hover:border-ink">{expanded === q.id ? 'Cerrar detalles' : 'Editar detalles'}</button>
                  ...
                  {q.jobId && <span className="inline-flex items-center h-10 px-3 rounded-lg bg-green-50 text-green-800 text-sm font-bold">Ya convertida en servicio</span>}
                </div>
              </div>
            ...
            {expanded === q.id && <QuoteDetailForm q={q} onSave={(fields) => saveDetails(q.id, fields)} onCancel={() => setExpanded(null)} />}
          </article>
        ))}
      </div>
    </div>
